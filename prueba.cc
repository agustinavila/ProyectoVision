 ...
 * @brief Primer prueba, basandome en el ejemplo base de libfreenect2
 * @version 0.1
 * @date 2021-06-15
 * 
 * 
...
int main()
{
	...
	while(!protonect_shutdown)
	{
		...
		string strNameModified = "Frame "+ to_string(contador) + " rgb.jpg";
		imwrite(strNameModified, rgbmat);
		cv::Mat(ir->height, ir->width, CV_32FC1, ir->data).copyTo(irmat);
		strNameModified = "Frame "+ to_string(contador) + " ir.jpg";
		imwrite(strNameModified, irmat);
		cv::Mat(depth->height, depth->width, CV_32FC1, depth->data).copyTo(depthmat);
		strNameModified = "Frame "+ to_string(contador) + " depth.jpg";
		imwrite(strNameModified, depthmat);
